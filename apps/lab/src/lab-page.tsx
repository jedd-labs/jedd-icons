...
export function LabPage() {
  ...
  return (
    <div className="min-h-svh bg-background text-foreground">
      <div className="mx-auto max-w-6xl border-border border-x">
        {/* Header */}
        <header className="flex items-baseline justify-between border-border border-b px-6 py-6">
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <aside className="shrink-0 border-border border-b lg:w-56 lg:border-r lg:border-b-0">
            <div className="sticky top-0 p-3">
              ...
              {/* Margin-violation filter. Disabled when nothing violates. */}
              <button
                className={`mb-2 flex w-full items-center gap-2 border px-2 py-1.5 text-left text-xs transition-colors ${
                  marginOnly
                    ? "border-rose-500 bg-rose-500/10 text-rose-600 dark:text-rose-400"
                    : "border-border text-muted-foreground hover:bg-muted/40 hover:text-foreground"
                } disabled:cursor-not-allowed disabled:opacity-50`}
                disabled={violating.size === 0}
                onClick={() => setMarginOnly((v) => !v)}
                type="button"
              >
                <span
                  aria-hidden
                ...
                />
                <span className="flex-1">Margin issues only</span>
                <span className="tabular-nums">{violating.size}</span>
              </button>
              ...
              <div className="max-h-[70vh] overflow-y-auto pr-1">
                {filtered.length === 0 ? (
                  <p className="px-2 py-4 text-muted-foreground text-xs">
                    {marginOnly
                      ? "No icons break the margin."
                      : "No icons match."}
                  </p>
                ) : (
                  <ul className="flex flex-col gap-0.5">
                    {filtered.map((name) => {
                      const flagged = violating.has(name);
                      return (
                        <li key={name}>
                          <button
          ...
          {/* ── Center: canvas ────────────────────────────────── */}
          <main className="flex min-w-0 flex-1 flex-col items-center gap-6 border-border border-b p-6 lg:border-r lg:border-b-0">
            ...
            {node ? (
              <div className="flex items-center justify-center overflow-auto rounded-none border border-border bg-muted/10 p-4">
                <LabCanvas
                  fill={variant === "fill"}
